...
contract("Air Drop", (accounts) => {
  ...
  before(async () => {
    ...
    await airDropContractInstance.updateMerkleRoot(merkleRoot, {
      from: creator,
    });
  ...
  it("01 - customer should be able to redeem tokens", async () => {
    ...
    const initialCustomerBalance = await tokenContractInstance.balanceOf(
      account01
    );
    const initialContractBalance = await tokenContractInstance.balanceOf(
      airDropAddress
    );
    ...
    truffleAssert.eventEmitted(tx, "Redeem", (obj) => {
      return obj.account === account01 && obj.amount.eq(amount);
    });
    ...
    const newContractBalance = await tokenContractInstance.balanceOf(
      airDropAddress
    );
  ...
  it("05 - it should be able to update Merkle Root", async () => {
    ...
    await airDropContractInstance.updateMerkleRoot(newMerkleRoot, {
      from: creator,
    });
    const updatedMerkleRoot =
      await airDropContractInstance.airDropWhiteListMerkleRoot.call();
